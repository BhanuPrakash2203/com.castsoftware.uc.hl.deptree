package AnaCoffeescript;
use strict;
use warnings;
use Erreurs;
use StripCoffeescript;
use AnaUtils;
use Vues;
use Timeout;
...
sub Strip($$$$)
{
  ...
  eval
  {
    $status = StripCoffeescript::StripCoffeescript($fichier, $vue, $options, $couples);
  };
  if ($@ )
  {
...
sub FileTypeRegister ($)
{
  ...
  if ($firstFile != 0)
  {
        ...
        my $ConfigModul='Coffeescript_Conf';
        ...
        {
            IsoscopeDataFile::csv_file_type_register("Coffeescript", $r_TableMnemos);
        }
        
        #------------------ init CloudReady detections -----------------------
		# if (defined $options->{'--CloudReady'}) {
			# CloudReady::detection::init($options, 'Coffeescript');
		# }
  }
}
