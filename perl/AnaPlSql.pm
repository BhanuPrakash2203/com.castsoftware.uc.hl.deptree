...
# Composant: Plugin
# Description: Module d'analyse pour le langage PLSQL
...
sub Strip ($$$$)
{
  ...
  $status |= StripPlSql::StripPlSql ($fichier, $vue, $options, $couples);
...
sub Parse ($$$$)
{
  ...
  eval
  {
    $status |= PlSql::Parse::Parse ($fichier, $vue, $couples, $options);
...
(
 ...
 # Comptages recuperes
  [\&PlSql::Parse::Count_Methods, 'Parse::Count_Methods'],
  [\&PlSql::Parse::Count_Parameters, 'Parse::Count_Parameters'],

 # Comptages PlSql
  [\&PlSql::CountKeywords::CountKeywords, 'CountKeywords::CountKeywords'],
  [\&PlSql::CountCommentedOutCode::CountCommentedOutCode, 'PlSql::CountCommentedOutCode'],
  [\&PlSql::CountFunctionsMethodsAttributes::CountFunctionsMethodsAttributes, 'PlSql::CountFunctionsMethodsAttributes'],
  [\&PlSql::CountBody::CountBody, 'PlSql::CountBody'],
  [\&PlSql::CountMultipleInstructions::CountMultipleInstructions, 'PlSql::CountMultipleInstructions'],
  [\&PlSql::CountConditionsAndOr::CountConditionsAndOr, 'PlSql::CountConditionsAndOr'],
  [\&PlSql::CountComplexConditions::CountComplexConditions, 'PlSql::CountComplexConditions'],
  [\&PlSql::CountWhenOthers::CountWhenOthers, 'PlSql::CountWhenOthers'],
  [\&PlSql::CountLocalVariables::CountLocalVariables, 'PlSql::CountLocalVariables'],
  [\&PlSql::CountExceptionWhen::CountExceptionWhen, 'PlSql::CountExceptionWhen'],
  [\&PlSql::CountWords::CountWords, 'PlSql::CountWords'],
...
sub Count ($$$$$)
{
    ...
    # Split into several artifacts units should be realized only if :
    # - at lest one artifacts is present
    # - it is not a pkb or pks file (i.e. file belonging to a packaged application))
    #if ((defined $options->{'--split-unit'}) && ($fichier =~ /\.sql$/) && (PlSql::Parse::containsArtifact($vue))) {
    if ((defined $options->{'--no-unit'}) || (! PlSql::Parse::containsArtifact($vue))) {
		$status = AnaUtils::Count ($fichier, $vue, $options, $couples, $r_TableFonctions);
	}
    else {
		$status = AnaUtils::CountForSeveralUnits ($fichier, $vue, $options, $couples, $r_TableFonctions, \&PlSql::Parse::getNextUnit);
...
sub FileTypeRegister ($)
{
  ...
  if ($firstFile != 0)
  {
        ...
        {
            IsoscopeDataFile::csv_file_type_register("PlSql", $r_TableMnemos);
        }
  }
}
