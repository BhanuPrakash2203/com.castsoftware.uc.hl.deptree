...
StripUtils::init('StripMariaDB', 0);
...
sub StripMariaDB($$$$)
{
    ...
    configureLocalTraces('StripMariaDB', $options); # traces_filter_line
    my $stripMariaDBTiming = new Timing ('StripMariaDB', Timing->isSelectedTiming ('Strip'));
    $stripMariaDBTiming->markTimeAndPrint('--init--') if ($b_timing_strip); # timing_filter_line
    
    my $err = parseCode(\$vue->{'text'}, $options);
    
    $stripMariaDBTiming->markTimeAndPrint('separer_code_commentaire_chaine') if ($b_timing_strip); # timing_filter_line
    ...
    if ( $err gt 0) {
     ...
     if ($CODE !~ /\S/s ) {
      return Erreurs::FatalError( Erreurs::ABORT_CAUSE_NOT_ENOUGH_CODE, $couples, "No MariaDB found");
    }
    ...
    $stripMariaDBTiming->dump('StripMariaDB') if ($b_timing_strip); # timing_filter_line
    return 0;
}
