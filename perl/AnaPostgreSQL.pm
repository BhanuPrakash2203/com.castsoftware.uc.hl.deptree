...
sub Strip($$$$)
{
  ...
  eval
  {
    $status = StripPostgreSQL::StripPostgreSQL($fichier, $vue, $options, $couples);
  };
  if ($@ )
  {
...
sub FileTypeRegister ($)
{
  ...
  if ($firstFile != 0)
  {
        ...
        my $ConfigModul='PostgreSQL_Conf';
        ...
        {
            IsoscopeDataFile::csv_file_type_register("PostgreSQL", $r_TableMnemos);
        }
        
        #------------------ init CloudReady detections -----------------------
		# if (defined $options->{'--CloudReady'}) {
			# CloudReady::detection::init($options, 'PostgreSQL');
		# }
  }
}
