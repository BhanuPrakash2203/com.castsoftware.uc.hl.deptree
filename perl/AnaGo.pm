...
sub Strip($$$$)
{
  ...
  eval
  {
    $status = StripGo::StripGo($fichier, $vue, $options, $couples);
  };
  if ($@ )
  {
...
sub Parse($$$$) {
	...
	my $status = 0;
	eval {
		$status |= Go::ParseGo::Parse($fichier, $vue, $compteurs, $options);
	};
	if ($@ ) {
		Timeout::DontCatchTimeout();   # propagate timeout errors
...
sub FileTypeRegister ($)
{
  ...
  if ($firstFile != 0)
  {
        ...
        {
            IsoscopeDataFile::csv_file_type_register("Go", $r_TableMnemos);
        }
        
        #------------------ init CloudReady detections -----------------------
		# if (defined $options->{'--CloudReady'}) {
			# CloudReady::detection::init($options, 'Go');
		# }
  }
}
