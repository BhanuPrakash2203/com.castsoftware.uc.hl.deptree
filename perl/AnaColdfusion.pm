...
sub Strip($$$$)
{
  ...
  eval
  {
    $status = StripColdfusion::StripColdfusion($fichier, $vue, $options, $couples);
  };
  if ($@ )
  {
...
sub FileTypeRegister ($)
{
  ...
  if ($firstFile != 0)
  {
        ...
        my $ConfigModul='Coldfusion_Conf';
        ...
        {
            IsoscopeDataFile::csv_file_type_register("Coldfusion", $r_TableMnemos);
        }
        
        #------------------ init CloudReady detections -----------------------
		# if (defined $options->{'--CloudReady'}) {
			# CloudReady::detection::init($options, 'Coldfusion');
		# }
  }
}
