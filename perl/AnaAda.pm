...
sub Strip($$$$)
{
  ...
  eval
  {
    $status = StripAda::StripAda($fichier, $vue, $options, $couples);
  };
  if ($@ )
  {
...
sub FileTypeRegister ($)
{
  ...
  if ($firstFile != 0)
  {
        ...
        {
            IsoscopeDataFile::csv_file_type_register("Ada", $r_TableMnemos);
        }
        
        #------------------ init CloudReady detections -----------------------
		# if (defined $options->{'--CloudReady'}) {
			# CloudReady::detection::init($options, 'Ada');
		# }
  }
}
