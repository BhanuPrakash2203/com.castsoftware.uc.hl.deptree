...
sub StripIMSDC($$$$)
{
	...
	configureLocalTraces('StripIMSDC', $options); # traces_filter_line
	my $stripIMSDCTiming = new Timing ('StripIMSDC', Timing->isSelectedTiming ('Strip'));
	$stripIMSDCTiming->markTimeAndPrint('--init--') if ($b_timing_strip); # timing_filter_line

	my $err = parseCode(\$vue->{'text'}, $options);

	$stripIMSDCTiming->markTimeAndPrint('separer_code_commentaire_chaine') if ($b_timing_strip); # timing_filter_line
	...
	if ($CODE !~ /\S/s ) {
		return Erreurs::FatalError( Erreurs::ABORT_CAUSE_NOT_ENOUGH_CODE, $couples, "No IMSDC found");
	}
	...
	$stripIMSDCTiming->dump('StripIMSDC') if ($b_timing_strip); # timing_filter_line
	return 0;
}
