...
use Erreurs;
use StripKotlin;
use Kotlin::ParseKotlin;
use AnaUtils;
use Vues;
use Timeout;
use IsoscopeDataFile;

use CloudReady::CountKotlin;
...
sub Strip($$$$)
{
  ...
  eval
  {
    $status = StripKotlin::StripKotlin($fichier, $vue, $options, $couples);
  };
  if ($@ )
  {
    Timeout::DontCatchTimeout();   # propagate timeout errors
    print STDERR "Strip error : $@ \n" ;
...
# }
 ...
 sub Count($$$$$) {
	 ...
	 if (defined $options->{'--CloudReady'}) {
		 ...
		 $status |= CloudReady::CountKotlin::CountKotlin( $fichier, $vue, $options);
	 }
     return $status;
 }
...
sub FileTypeRegister ($)
{
  ...
  if ($firstFile != 0)
  {
        ...
        {
            IsoscopeDataFile::csv_file_type_register("Kotlin", $r_TableMnemos);
        }
        
        #------------------ init CloudReady detections -----------------------
		if (defined $options->{'--CloudReady'}) {
			CloudReady::detection::init($options, 'Kotlin');
		}
  }
}
...
sub Analyse($$$$)
{
  ...
  my $analyseur_callbacks = [ \&Strip, \&Kotlin::ParseKotlin::Parse, \&Count, $r_TableFonctions ];
