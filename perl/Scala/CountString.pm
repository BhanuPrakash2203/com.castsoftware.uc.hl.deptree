...
sub CountString($$$)
{
    ...
    for my $id (keys %countID) {
        if (exists $HString->{$id}) {
            # HL-1973 21/03/2022 String literals should not be duplicated
            # String is enclosed by double quotes " so counter is computed as (2 char + 2 quotes = 4)
            if (length $HString->{$id} > 4 && $countID{$id} > 2) {
                # print "String literals $HString->{$id} should not be duplicated more than 2 times\n";
                $nb_DuplicatedString++;
