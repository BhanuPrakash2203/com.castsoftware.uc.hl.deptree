...
sub separer_code_commentaire_chaine($$$)
{
    ...
    my $stripPlSqlTiming = new Timing ('StripTSql:separer_code_commentaire_chaine', Timing->isSelectedTiming ('Strip'));
    my $c = $$source;
    my $state = 'INCODE' ;
    my $f = $state ;
    ...
    my $stripPlSqlTimingLoop = new Timing ('StripTSql internal Loop', Timing->isSelectedTiming ('Strip'));
...
# FIXME: le decoupage ne s'effectue pas correctement sur as, is, etc, car correspond aussi au tout sauf accolade...
  ...
  # FIXME : Keywords can be used as variable identifiers if double-quoted.
  #         The split does not take this in account. Such variables names
  #         should be neutralized before the split step.
  my @statements_with_blanks =  split ( 
    /(;|\b(?:go|begin\s+(?:transaction|tran|try|catch)|end\s+(?:try|catch)|begin|end|while|if|return|else|select|union\s*(?:all)?|intersect|except|declare|create\s+(?:proc|procedure|trigger|function)|break|continue|(?:commit|rollback)\s+(?:transaction|tran)|bulk\s+insert|truncate\s+table|create|alter|drop|insert|update|merge|delete|fetch)\b|\bgrant\b.*?\bto\b|(?:\w+ *:))/smi, 
...
sub StripTSql($$$$)
{
    ...
    my $b_timing_strip = Timing->isSelectedTiming ('Strip')   ;                 # timing_filter_line
#print STDERR join ( "\n", keys ( %{$options} ) );
    configureLocalTraces('StripTSql', $options);                               # traces_filter_line
    my $stripPlSqlTiming = new Timing ('StripTSql', $b_timing_strip);          # timing_filter_line
...
#    return ErrStripError()  if ($err gt 0);
    ...
    if ( not Erreurs::isAborted($err) )
    {
      ...
      $stripPlSqlTiming->dump('StripTSql') if ($b_timing_strip);                 # timing_filter_line
