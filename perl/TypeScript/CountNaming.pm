...
package TypeScript::CountNaming ;
...
sub checkParameterNaming($) {
	...
	for my $param (@$r_paramList) {
#print "PARAM $param->[0] (".length($param->[0]).")\n";
		$SumSizeParametersNames += length $param->[0];
		$nb_TotalParameters++;
	}
}
...
sub CountNaming($$$) {
  ...
  push @funcs, @{$vue->{'KindsLists'}->{'Method'}};

  for my $func (@funcs) {

    my $paramList = Lib::NodeUtil::GetXKindData($func, 'parameters');
...
#print "FUNCTION ".($name||"??")."\n";
  ...
  # FUNCTION EXPRESSION
  # -----------------------------------------------------------------------
  for my $func (@{$vue->{'KindsLists'}->{'FunctionExpression'}}) {
#print "FUNCTION EXPRESSION\n";
  ...
  # Extends Properties to class and interfaces
  my @props = GetNodesByKindList($root, [MemberKind, AttributeKind]);
  
  my %found = ();
  for my $prop (@props) {
    my $name = GetName($prop);
	
	next if (!defined $name) || (exists $found{$name});
...
#-------------------------------------------------------------------------
  ...
  Erreurs::VIOLATION($mnemo_ShortParameterNamesLT, "METRIC : parameter length average = $nb_ShortParameterNamesLT");
