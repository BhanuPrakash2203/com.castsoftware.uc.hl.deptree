package TypeScript::CountFunction;
...
sub countBadSpacingInFuncDecl($) {
    ...
    # FIXME: To be validated : Words function|get|set are optional (sugar syntax)
    ${GetStatement($func)} =~ /^\s*(?:(?:function|get|set))?(\s+$IDENTIFIER)?(\s*)\(?/m;

    if ((defined $1) && ($2 ne '')) {
        # if there is one or more space between the name and the openning parenthesis, raise a violation.
        # print "---> BadSpacing violation: one or more spaces between name and opening parenthesis\n";
        return 1;
    }
    elsif ( (! defined $1) && ($2 eq '')) {
        # if it is an anonymous function and there is no space after the 'function' keyword, raise an violation.
        # print "---> BadSpacing violation: anonymous func and no space between function and opening parenthesis\n";
        return 1;
    }
...
sub getNodeDepth($) 
{
  ...
  for my $child (@{$Children}) {
    ...
    # skip inner artifacts ...
    if (($kind ne FunctionDeclarationKind) && ($kind ne FunctionExpressionKind) && ($kind ne ClassDeclarationKind) && ($kind ne ClassExpressionKind)) {
...
#print "---> Misplaced var\n";
	$varZone = 1;
      ...
      if ( (!IsKind($child, FunctionDeclarationKind)) &&
        ...
        my @VarInSubnodes = GetNodesByKindList_StopAtBlockingNode($child,	[VarKind, ForInitKind], [FunctionDeclarationKind, FunctionExpressionKind, ClassDeclarationKind, ClassExpressionKind], 1);
...
sub isTooDepthArtifact($) {
  ...
  if (getNodeDepth($func) > $TypeScript::TypeScriptConfig::ARTIFACT_MAX_DEPTH ) {
#print "TO DEPTH !!!\n";
      return 1;
  }
  return 0;
...
sub isComplexArtifact($$;$) {
  ...
  if (! defined $THRESHOLD) {
    $THRESHOLD = $TypeScript::TypeScriptConfig::ComplexArtifact__THRESHOLD;
  }
...
sub CountFunction($$$) 
{
    ...
    for my $func (@funcs) {
        ...
        if ($parentKind ne RootKind) {
            if ($parentKind eq AccoKind) {
				...
				# PARENT is an ACCOLADE
				
                my $granParent = GetParent($parent);
                my $granParentKind = GetKind($granParent);
                #print "--> GRAN-PARENT = ".$granParentKind."\n";
                if ( ! (($granParentKind eq FunctionDeclarationKind) || ($granParentKind eq FunctionExpressionKind) || ($granParentKind eq MethodKind)) ) {
					#print " -----> Violation !!!\n";
					# if the parent is not root nor an accolade, then the grand-parent cannot be a function (because a function as always an accolade block as first child),
					# then raise a violation (because it is then a non-function block))
					$nb_WithinBlocksFunctionsDecl++;
					Erreurs::VIOLATION($WithinBlocksFunctionsDecl__mnemo, "Function declared inside accolade block at line ".GetLine($func).".");
                }
            }
            else {
				
				# PARENT is not an ACCOLADE nor root
				
                # if the parent is not an accolade (so the grand-parent cannot be a function) , then raise a violation.
                #print " -----> Violation !!!\n";
                $nb_WithinBlocksFunctionsDecl++;
                Erreurs::VIOLATION($WithinBlocksFunctionsDecl__mnemo, "Function declared inside block (nor root, nor accolde!) at line ".GetLine($func).".");
            }
        }
        ...
        $nb_BadReturnStatement += checkReturnStatement($func);
    }
	...
	# METHOD
	# -----------------------------------------------------------------------
	@funcs = @{$vue->{'KindsLists'}->{'Method'}};

	$nb_MethodImplementations += scalar @funcs;

	for my $meth (@funcs) {
#print "FUNCTION : ".GetName($func)."\n";
		$nb_ComplexArtifact += isComplexArtifact($meth, $artifactView);
		$nb_MisplacedInnerFuncDecl += countMisplacedInnerFuncDecl($meth, $artifactView);
		$nb_BadSpacingInFuncDecl += countBadSpacingInFuncDecl($meth);
		$nb_TooDepthArtifact += isTooDepthArtifact($meth);

		# function have an "acco" level as first child, instructions of the function
		# are childs of the acco node.
		$nb_MisplacedVarStatement+= hasMisplacedVar(firstChild($meth));
	}
...
#print "--->stmt = $$stmt \n";
	...
	my $tag = TypeScript::TypeScriptNode::nodeTag($previous);
	if ($$stmt !~ /\A\s*$tag/) {
...
# HL-858 26/04/2019 Function returns should not be invariant
sub checkReturnStatement($)
{
    my $func = shift;
    my $counterViolation = 0;
    my %returnStmt;
   
    my @returns = Lib::NodeUtil::GetNodesByKind($func, ReturnKind);
     
    for my $return (@returns)
    {
        my $statement = ${GetStatement($return)};
        $statement =~ s/\s+//g;
        
        if (exists $returnStmt{$statement})
        {
            print "Function returns should not be invariant at line ".GetLine($return)."\n" if $DEBUG;
            $counterViolation++;
            Erreurs::VIOLATION($BadReturnStatement__mnemo, "Function returns should not be invariant at line ".GetLine($return));
        }
        else
        {
