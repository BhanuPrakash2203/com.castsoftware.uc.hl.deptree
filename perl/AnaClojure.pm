...
sub Strip($$$$)
{
  ...
  eval
  {
    $status = StripClojure::StripClojure($fichier, $vue, $options, $couples);
  };
  if ($@ )
  {
 ...
 sub Parse($$$$) {
	 ...
	 my $status = 0;
	 eval {
		 $status |= Clojure::ParseClojure::Parse($fichier, $vue, $compteurs, $options);
	 };
	 if ($@ ) {
		 Timeout::DontCatchTimeout();   # propagate timeout errors
 ...
 sub Count($$$$$) {
	 ...
	 if (defined $options->{'--CloudReady'}) {
		 ...
		 $status |= CloudReady::CountClojure::CountClojure( $fichier, $vue, $options);
	 }
     return $status;
 }
...
sub FileTypeRegister ($)
{
  ...
  if ($firstFile != 0)
  {
        ...
        {
            IsoscopeDataFile::csv_file_type_register("Clojure", $r_TableMnemos);
        }
        
        #------------------ init CloudReady detections -----------------------
		if (defined $options->{'--CloudReady'}) {
			CloudReady::detection::init($options, 'Clojure');
		}
  }
}
