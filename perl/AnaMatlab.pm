...
sub Strip($$$$)
{
  ...
  eval
  {
    $status = StripMatlab::StripMatlab($fichier, $vue, $options, $couples);
  };
  if ($@ )
  {
...
sub FileTypeRegister ($)
{
  ...
  if ($firstFile != 0)
  {
        ...
        {
            IsoscopeDataFile::csv_file_type_register("Matlab", $r_TableMnemos);
        }
        
        #------------------ init CloudReady detections -----------------------
		# if (defined $options->{'--CloudReady'}) {
			# CloudReady::detection::init($options, 'Matlab');
		# }
  }
}
